...
uint64 sys_user_allocate_page(uint64 nb) {
  ...
  while (NULL != md && md->size < nb) md = md->next;
  if (NULL == md) { // no single satisfying memory_descriptor
    ...
    while(NULL != md && (md->page_start_va != current->user_free_va - PGSIZE 
          || md->start_pa + md->size - md->page_start_pa != PGSIZE)) {
      md = md->next;
    }
...
void free_single_md(memory_descriptor *md) {
  ...
  if (0 == --current->page_cb[page_id].allocated_md_num) { // free the whole page
    ...
    // md->size += current->page_cb[page_id].allocated_md_num;
  }
}
